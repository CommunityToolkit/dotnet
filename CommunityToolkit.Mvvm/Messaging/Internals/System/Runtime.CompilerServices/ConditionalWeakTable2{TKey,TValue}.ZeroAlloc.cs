...
internal sealed class ConditionalWeakTable<TKey, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)]TValue> : IEnumerable<KeyValuePair<TKey, TValue>>
    where TKey : class
    where TValue : class?
{
    ...
    private sealed class Enumerator : IEnumerator<KeyValuePair<TKey, TValue>>
    {
        ...
        public KeyValuePair<TKey, TValue> Current
        {
            get
            {
                if (_currentIndex < 0)
                {
                    throw new InvalidOperationException("No enumeration");
                }
                return _current;
            }
        }
