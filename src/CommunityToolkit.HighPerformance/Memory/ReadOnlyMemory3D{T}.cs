...
/// <summary>
/// A readonly version of <see cref="Memory3D{T}"/>.
/// </summary>
/// <typeparam name="T">The type of items in the current <see cref="ReadOnlyMemory3D{T}"/> instance.</typeparam>
[DebuggerTypeProxy(typeof(MemoryDebugView3D<>))]
[DebuggerDisplay("{ToString(),raw}")]
public readonly struct ReadOnlyMemory3D<T> : IEquatable<ReadOnlyMemory3D<T>>
{
    /// <summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReadOnlyMemory3D{T}"/> struct.
    /// </summary>
    ...
    /// </exception>
    /// <remarks>The total volume must match the length of <paramref name="text"/>.</remarks>
    public ReadOnlyMemory3D(string text, int depth, int height, int width)
        : this(text, 0, depth, height, width, slicePitch: 0, rowPitch: 0)
    {
    }
    ...
    /// </exception>
    /// <exception cref="ArgumentException">
    /// Thrown when the requested volume is outside of bounds for <paramref name="text"/>.
    /// </exception>
    public ReadOnlyMemory3D(string text, int offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
    ...
    public ReadOnlyMemory3D(T[] array, int depth, int height, int width)
    ...
    /// </exception>
    public ReadOnlyMemory3D(T[] array, int offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReadOnlyMemory3D{T}"/> struct wrapping a 3D array.
    /// </summary>
    /// <param name="array">The given 3D array to wrap.</param>
    public ReadOnlyMemory3D(T[,,]? array)
    {
        if (array is null)
        {
            this = default;
    ...
    /// </exception>
    public ReadOnlyMemory3D(T[,,]? array, int slice, int row, int column, int depth, int height, int width)
    {
        if (array is null)
        {
    ...
    public ReadOnlyMemory3D(MemoryManager<T> memoryManager, int depth, int height, int width)
    ...
    /// </exception>
    public unsafe ReadOnlyMemory3D(MemoryManager<T> memoryManager, int offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
    ...
    internal ReadOnlyMemory3D(ReadOnlyMemory<T> memory, int depth, int height, int width)
    ...
    /// </exception>
    internal unsafe ReadOnlyMemory3D(ReadOnlyMemory<T> memory, int offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
        ...
        // Check if the ReadOnlyMemory<T> instance wraps a string. This is possible in case
        // consumers do an unsafe cast for the entire ReadOnlyMemory<T> object, and while not
        ...
        {
            // Check if the input ReadOnlyMemory<T> instance wraps an array we can access.
            // This is fine, since ReadOnlyMemory<T> on its own doesn't control the lifetime
            // of the underlying array anyway, and this ReadOnlyMemory3D<T> type would do the same.
            // Using the array directly makes retrieving a ReadOnlySpan3D<T> faster down the line,
            // as we no longer have to jump through the boxed ReadOnlyMemory<T> first anymore.
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ReadOnlyMemory3D{T}"/> struct with the specified parameters.
    /// </summary>
    ...
    private ReadOnlyMemory3D(object instance, IntPtr offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
    ...
    /// <summary>
    /// Creates a new <see cref="ReadOnlyMemory3D{T}"/> instance from an arbitrary object.
    /// </summary>
    ...
    /// <param name="slicePitch">The pitch of each slice in the 3D memory area to map.</param>
    /// <param name="rowPitch">The pitch of each row in the 3D memory area to map.</param>
    /// <returns>A <see cref="ReadOnlyMemory3D{T}"/> instance with the specified parameters.</returns>
    ...
    /// Thrown when one of the input parameters is out of range.
    /// </exception>
    public static ReadOnlyMemory3D<T> DangerousCreate(object instance, ref T value, int depth, int height, int width, int slicePitch, int rowPitch)
    {
        if (depth < 0)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForDepth();
    ...
    /// <summary>
    /// Gets an empty <see cref="ReadOnlyMemory3D{T}"/> instance.
    /// </summary>
    public static ReadOnlyMemory3D<T> Empty => default;

    /// <summary>
    /// Gets a value indicating whether the current <see cref="ReadOnlyMemory3D{T}"/> instance is empty.
    /// </summary>
    public bool IsEmpty
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Gets the length of the current <see cref="ReadOnlyMemory3D{T}"/> instance.
    /// </summary>
    public nint Length
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Gets a <see cref="ReadOnlySpan3D{T}"/> instance from the current memory.
    /// </summary>
    public ReadOnlySpan3D<T> Span
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
            if (this.instance is not null)
            {
                ...
                if (this.instance is MemoryManager<T> memoryManager)
                {
                    ...
                    return new(in r1, this.depth, this.height, this.width, this.slicePitch, this.rowPitch);
                }
                else
                {
                    ...
                    return new(in r0, this.depth, this.height, this.width, this.slicePitch, this.rowPitch);
                }
#else
    ...
    /// </exception>
    /// <returns>A new <see cref="ReadOnlyMemory3D{T}"/> instance representing a slice of the current one.</returns>
    public ReadOnlyMemory3D<T> this[Range slices, Range rows, Range columns]
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
    ...
    public unsafe ReadOnlyMemory3D<T> Slice(int slice, int row, int column, int depth, int height, int width)
    {
    ...
    /// <summary>
    /// Copies the contents of this <see cref="ReadOnlyMemory3D{T}"/> into a destination <see cref="Memory{T}"/> instance.
    /// </summary>
    /// <param name="destination">The destination <see cref="Memory{T}"/> instance.</param>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="destination" /> is shorter than the source <see cref="ReadOnlyMemory3D{T}"/> instance.
    /// </exception>
    public void CopyTo(Memory<T> destination) => Span.CopyTo(destination.Span);

    /// <summary>
    /// Attempts to copy the current <see cref="ReadOnlyMemory3D{T}"/> instance to a destination <see cref="Memory{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Copies the contents of this <see cref="ReadOnlyMemory3D{T}"/> into a destination <see cref="Memory3D{T}"/> instance.
    ...
    /// <summary>
    /// Attempts to copy the current <see cref="ReadOnlyMemory3D{T}"/> instance to a destination <see cref="Memory3D{T}"/>.
    /// For this API to succeed, the target <see cref="Memory3D{T}"/> has to have the same shape as the current one.
    /// </summary>
    ...
    public bool TryGetMemory(out ReadOnlyMemory<T> memory)
    {
        if (this.slicePitch == 0 &&
            ...
            Length <= int.MaxValue)
        {
            // Empty ReadOnlyMemory3D<T> instance
    ...
    /// <summary>
    /// Copies the contents of the current <see cref="ReadOnlyMemory3D{T}"/> instance into a new 3D array.
    /// </summary>
    /// <returns>A 3D array containing the data in the current <see cref="ReadOnlyMemory3D{T}"/> instance.</returns>
    ...
    /// <inheritdoc/>
    public bool Equals(ReadOnlyMemory3D<T> other)
    {
        return
    ...
    {
        return $"CommunityToolkit.HighPerformance.ReadOnlyMemory3D<{typeof(T)}>[{this.depth}, {this.height}, {this.width}]";
    }

    /// <summary>
    /// Defines an implicit conversion of an array to a <see cref="ReadOnlyMemory3D{T}"/>
    /// </summary>
    public static implicit operator ReadOnlyMemory3D<T>(T[,,]? array) => new(array);

    /// <summary>
    /// Defines an implicit conversion of a <see cref="Memory3D{T}"/> to a <see cref="ReadOnlyMemory3D{T}"/>
    /// </summary>
    public static implicit operator ReadOnlyMemory3D<T>(Memory3D<T> memory) => Unsafe.As<Memory3D<T>, ReadOnlyMemory3D<T>>(ref memory);
}