...
/// <summary>
/// <see cref="Span3D{T}"/> represents a 3D region of arbitrary memory. Like the <see cref="Span{T}"/> type,
/// it can point to either managed or native memory, or to memory allocated on the stack. It is type- and memory-safe.
/// One key difference with <see cref="Span{T}"/> and arrays is that the underlying buffer for a <see cref="Span3D{T}"/>
/// instance might not be contiguous in memory: this is supported to enable mapping arbitrary 3D regions even if they
/// require padding between boundaries of sequential rows. All this logic is handled internally by the <see cref="Span3D{T}"/>
/// type and it is transparent to the user, but note that working over discontiguous buffers has a performance impact.
/// </summary>
/// <typeparam name="T">The type of items in the current <see cref="Span3D{T}"/> instance.</typeparam>
[DebuggerTypeProxy(typeof(MemoryDebugView3D<>))]
[DebuggerDisplay("{ToString(),raw}")]
public readonly ref partial struct Span3D<T>
{
    // Let's consider a representation of a discontiguous 3D memory
    // region within an existing array. The data is represented as
    // an array of 2D slices, each in row-major order as usual, and
    // the 'XX' grid cells represent locations that are mapped by a
    // given Span3D<T> instance for one of those slices:
    //
    ...
    // discontiguous row in a slice, so that any arbitrary memory
    // locations can be used to internally represent a 3D span.
    // This gives users much more flexibility when creating spans
    // from data.
    //
    // This is also extended to 3D, where each slice has a slice pitch
    // that is used to calculate the offset between each discontinuous
    // slice in the 3D memory region.
    ...
    /// </summary>
    /// <remarks>
    /// The <see cref="Span{T}.Length"/> field maps to the depth of the 3D region.
    /// This is done to save 4 bytes in the layout of the <see cref="Span3D{T}"/> type.
    /// </remarks>
...
#if NETSTANDARD2_1_OR_GREATER
    /// <summary>
    /// Initializes a new instance of the <see cref="Span3D{T}"/> struct with the specified parameters.
    /// </summary>
    ...
    internal Span3D(ref T value, int depth, int height, int width, int slicePitch, int rowPitch)
    {
#if NET8_0_OR_GREATER
        this.reference = ref value;
        this.depth = depth;
#else
        this.span = MemoryMarshal.CreateSpan(ref value, depth);
#endif
    ...
    public unsafe Span3D(void* pointer, int depth, int height, int width, int slicePitch, int rowPitch)
    {
        ...
        this.depth = depth;
#elif NETSTANDARD2_1_OR_GREATER
        this.span = new Span<T>(pointer, depth);
#else
        this.Instance = null;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal Span3D(object? instance, IntPtr offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="Span3D{T}"/> struct.
    /// </summary>
    ...
    public Span3D(T[] array, int depth, int height, int width)
    ...
    /// Thrown when the requested volume is outside of bounds for <paramref name="array"/>.
    /// </exception>
    public Span3D(T[] array, int offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
        ...
        this.depth = depth;
#elif NETSTANDARD2_1_OR_GREATER
        this.span = MemoryMarshal.CreateSpan(ref array.DangerousGetReferenceAt(offset), depth);
#else
        this.Instance = array;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="Span3D{T}"/> struct wrapping a 3D array.
    /// </summary>
    ...
    /// </exception>
    public Span3D(T[,,]? array, int slice, int row, int column, int depth, int height, int width)
    {
        if (array is null)
        {
    ...
    internal Span3D(Span<T> span, int depth, int height, int width)
    ...
    /// </exception>
    internal Span3D(Span<T> span, int offset, int depth, int height, int width, int slicePitch, int rowPitch)
    {
    ...
    /// <summary>
    /// Creates a new instance of the <see cref="Span3D{T}"/> struct with the specified parameters.
    /// </summary>
    /// <param name="value">The reference to the first <typeparamref name="T"/> item to map.</param>
    /// <param name="depth">The depth of the 3D memory area to map.</param>
    /// <param name="height">The height of the 3D memory area to map.</param>
    ...
    /// <param name="slicePitch">The slice pitch of the 3D memory area to map (the distance between each slice).</param>
    /// <param name="rowPitch">The row pitch of the 3D memory area to map (the distance between each row).</param>
    /// <returns>A <see cref="Span3D{T}"/> instance with the specified parameters.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when one of the parameters are negative.</exception>
    public static Span3D<T> DangerousCreate(ref T value, int depth, int height, int width, int slicePitch, int rowPitch)
    {
        if (width < 0)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForWidth();
        ...
        return new(ref value, depth, height, width, slicePitch, rowPitch);
    }
#endif

    /// <summary>
    /// Gets an empty <see cref="Span3D{T}"/> instance.
    /// </summary>
    public static Span3D<T> Empty => default;

    /// <summary>
    /// Gets a value indicating whether the current <see cref="Span3D{T}"/> instance is empty.
    /// </summary>
    public bool IsEmpty
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Gets the length of the current <see cref="Span3D{T}"/> instance.
    /// </summary>
    public nint Length
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </exception>
    public ref T this[int slice, int row, int column]
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
    ...
    /// Thrown when either <paramref name="slice"/>, <paramref name="row"/>, or <paramref name="column"/> are invalid.
    /// </exception>
    public ref T this[Index slice, Index row, Index column]
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// </exception>
    /// <returns>A new <see cref="Span3D{T}"/> instance representing a slice of the current one.</returns>
    public Span3D<T> this[Range slices, Range rows, Range columns]
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get
        {
    ...
    /// <summary>
    /// Clears the contents of the current <see cref="Span3D{T}"/> instance.
    /// </summary>
    public void Clear()
    {
        if (IsEmpty)
        ...
        else
        {
            ...
            {
                // Clear one slice at a time
                for (int z = 0; z < depth; z++)
                {
                    GetSliceSpan(z).Clear();
                }
            }
            else
            {
                ...
                for (int z = 0; z < depth; z++)
                {
                    ...
                    {
                        ref T rStart = ref DangerousGetReferenceAt(z, y, 0);
    ...
    /// <summary>
    /// Copies the contents of this <see cref="Span3D{T}"/> into a destination <see cref="Span{T}"/> instance.
    /// </summary>
    /// <param name="destination">The destination <see cref="Span{T}"/> instance.</param>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="destination" /> is shorter than the source <see cref="Span3D{T}"/> instance.
    /// </exception>
    ...
    /// <summary>
    /// Copies the contents of this <see cref="Span3D{T}"/> into a destination <see cref="Span3D{T}"/> instance.
    ...
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="destination" /> does not have the same shape as the source <see cref="Span3D{T}"/> instance.
    /// </exception>
    ...
    /// <summary>
    /// Attempts to copy the current <see cref="Span3D{T}"/> instance to a destination <see cref="Span{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Attempts to copy the current <see cref="Span3D{T}"/> instance to a destination <see cref="Span3D{T}"/>.
    /// </summary>
    ...
    /// </summary>
    /// <param name="value">The value to assign to each element of the <see cref="Span3D{T}"/> instance.</param>
    ...
    /// <summary>
    /// Returns a reference to the 0th element of the <see cref="Span3D{T}"/> instance. If the current
    ...
    /// on an instance that has 3 slices, 2 rows, and 1 column will throw, rather than returning a new <see cref="Span3D{T}"/>
    ...
    /// </para>
    /// <para>
    /// This is by design, and it is due to the internal memory layout that <see cref="Span3D{T}"/> has. That is, in the case
    ...
    /// With <see cref="Span3D{T}"/>, on the other hand, it would be possible to slice an instance with a size of 0 in either axis,
    ...
    /// </para>
    /// </remarks>
    public unsafe Span3D<T> Slice(int slice, int row, int column, int depth, int height, int width)
    {
        if ((uint)slice >= Depth)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForSlice();
        ...
        return new(ref r0, depth, height, width, slicePitch, rowPitch);
#elif NETSTANDARD2_1_OR_GREATER
    ...
    /// <summary>
    /// Gets a <see cref="Span2D{T}"/> for a specified slice.
    /// </summary>
    ...
    /// <returns>The resulting slice <see cref="Span2D{T}"/>.</returns>
    public unsafe Span2D<T> GetSliceSpan(int slice)
    {
        if ((uint)slice >= (uint)Depth)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForSlice();
        ...
        return new(ref r0, this.height, this.width, rowPitch);
#else
        nint shift = (nint)(uint)slice * (nint)(uint)this.SliceStride;
        IntPtr offset = this.Offset + (shift * (nint)(uint)sizeof(T));
    ...
    /// <summary>
    /// Gets a <see cref="Span{T}"/> for a specified row in a specific slice.
    /// </summary>
    /// <param name="slice">The index of the target slice to retrieve.</param>
    ...
    public unsafe Span<T> GetRowSpan(int slice, int row)
    {
        if ((uint)slice >= (uint)Depth)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForSlice();
        ...
        {
            return new Span<T>((void*)(this.Offset + (shift * (nint)(uint)sizeof(T))), this.width);
        }
    ...
    public bool TryGetSpan(out Span<T> span)
    {
            ...
            Length <= int.MaxValue)
        {
            ...
            return true;
#else
            // An empty Span3D<T> is still valid
            if (IsEmpty)
            {
                span = default;
    ...
    /// <summary>
    /// Copies the contents of the current <see cref="Span3D{T}"/> instance into a new 3D array.
    /// </summary>
    /// <returns>A 3D array containing the data in the current <see cref="Span3D{T}"/> instance.</returns>
    public T[,,] ToArray()
    {
    ...
    {
        throw new NotSupportedException("CommunityToolkit.HighPerformance.Span3D<T>.Equals(object) is not supported.");
    }
    ...
    {
        throw new NotSupportedException("CommunityToolkit.HighPerformance.Span3D<T>.GetHashCode() is not supported.");
    }

    /// <inheritdoc/>
    public override string ToString()
    {
        return $"CommunityToolkit.HighPerformance.Span3D<{typeof(T)}>[{Depth}, {this.height}, {this.width}]";
    }

    /// <summary>
    /// Checks whether two <see cref="Span3D{T}"/> instances are equal.
    /// </summary>
    /// <param name="left">The first <see cref="Span3D{T}"/> instance to compare.</param>
    /// <param name="right">The second <see cref="Span3D{T}"/> instance to compare.</param>
    /// <returns>Whether or not <paramref name="left"/> and <paramref name="right"/> are equal.</returns>
    public static bool operator ==(Span3D<T> left, Span3D<T> right)
    {
        return
#if NET8_0_OR_GREATER
    ...
    /// <summary>
    /// Checks whether two <see cref="Span3D{T}"/> instances are not equal.
    /// </summary>
    /// <param name="left">The first <see cref="Span3D{T}"/> instance to compare.</param>
    /// <param name="right">The second <see cref="Span3D{T}"/> instance to compare.</param>
    ...
    /// <summary>
    /// Implicitly converts a given 3D array into a <see cref="Span3D{T}"/> instance.
    /// </summary>
