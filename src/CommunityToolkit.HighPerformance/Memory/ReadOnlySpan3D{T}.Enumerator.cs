...
/// <inheritdoc/>
partial struct ReadOnlySpan3D<T>
{
    /// <summary>
    /// Gets an enumerable that traverses items in a specified row of a specific slice.
    /// </summary>
    /// <param name="slice">The target slice to enumerate within the current <see cref="ReadOnlySpan3D{T}"/> instance.</param>
    /// <param name="row">The target row to enumerate within the current <see cref="ReadOnlySpan3D{T}"/> instance.</param>
    ...
    public ReadOnlyRefEnumerable<T> GetRow(int slice, int row)
    {
        if ((uint)slice >= Depth)
        {
        ...
        return new(in r1, length: this.width, step: 1);
#else
    ...
    /// </summary>
    /// <param name="slice">The target slice to enumerate within the current <see cref="ReadOnlySpan3D{T}"/> instance.</param>
    /// <param name="column">The target column to enumerate within the current <see cref="ReadOnlySpan3D{T}"/> instance.</param>
    /// <returns>A <see cref="ReadOnlyRefEnumerable{T}"/> with target items to enumerate.</returns>
    /// <remarks>The returned <see cref="ReadOnlyRefEnumerable{T}"/> value shouldn't be used directly: use this extension in a <see langword="foreach"/> loop.</remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ReadOnlyRefEnumerable<T> GetColumn(int slice,int column)
    {
        if ((uint)slice >= Depth)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForSlice();
...
#if NETSTANDARD2_1_OR_GREATER
        return new(in r1, length: this.height, step: this.RowStride);
#else
        IntPtr offset = RuntimeHelpers.GetObjectDataOrReferenceByteOffset(this.instance, ref r1);
    ...
    /// <remarks>The returned <see cref="ReadOnlyRefEnumerable{T}"/> value shouldn't be used directly: use this extension in a <see langword="foreach"/> loop.</remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ReadOnlyRefEnumerable<T> GetDepthColumn(int row, int column)
    {
        if ((uint)row >= this.height)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForRow();
...
#if NETSTANDARD2_1_OR_GREATER
        return new(in r1, length: Depth, step: this.SliceStride);
#else
        IntPtr offset = RuntimeHelpers.GetObjectDataOrReferenceByteOffset(this.instance, ref r1);
    ...
    /// <summary>
    /// Returns an enumerator for the current <see cref="ReadOnlySpan3D{T}"/> instance.
    /// </summary>
    /// <returns>
    /// An enumerator that can be used to traverse the items in the current <see cref="ReadOnlySpan3D{T}"/> instance
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Enumerator GetEnumerator() => new(this);

    /// <summary>
    /// Provides an enumerator for the elements of a <see cref="ReadOnlySpan3D{T}"/> instance.
    /// </summary>
    public ref struct Enumerator
    {
        ...
        /// </summary>
        /// <remarks>Just like in <see cref="ReadOnlySpan3D{T}"/>, the length is the depth of the 3D region.</remarks>
        ...
        /// </summary>
        /// <param name="span">The target <see cref="ReadOnlySpan3D{T}"/> instance to enumerate.</param>
